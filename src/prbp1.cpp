...
namespace std {
ostream &operator<<(ostream &o, const MtSlack &m) { return o << "Sl[" << m.m << "," << m.s << "]"; }
} 
...
void reportLoad(const vector<Task> &L, const vector<short> &π, Time l) {
  fmt::print("Task order ");
  for (unsigned i = 0, ie = L.size(); i != ie; i++)
    fmt::print("{} ", L[π[i]] + 1);
  fmt::print("has load {}\n", l);
}
